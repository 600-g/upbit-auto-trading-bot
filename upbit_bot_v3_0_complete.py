...
- 절대 거짓 정보를 만들어내지 마"""
    ...
    def run_trading_cycle(self):
        ...
        for coin in selected:
            ...
            if momentum >= min_score or (coin_is_strong and momentum >= 30):
                # v6.4: 모멘텀 구간별 예산 (데이터: 20~25점 승률46.5% 최고, 30~40점 25.9% 최악)
                is_concentrate = False
                if momentum >= 60:
                    coin_budget = min(int(per_coin_budget * 1.5), available)
                    print(f" 🔥 고모멘텀({momentum}점) 확대 → {coin_budget:,}원", end="")
                elif 30 <= momentum < 40:
                    coin_budget = min(per_coin_budget // 2, available)
                    print(f" ⚠️ 과열구간({momentum}점) 예산 절반 → {coin_budget:,}원", end="")
                else:
    ...
    def _detect_surge_coins(self):
        """v5.25: surge 감지 — 모멘텀 점수 없이 3조건(방향+힘+타이밍)만으로 판단

        설계 원칙:
        - 모멘텀 점수는 batch용(앞으로 오를까?), surge는 "지금 올라가고 있나?"만 봄
        - RSI/고점/추세 패널티 없음 — 올라가는 코인이니까 당연히 RSI 높고 고점 근처
        - 3조건: ① 5분봉 양봉(방향) ② 거래량 2배+(힘) ③ 10분 내 +2%+(타이밍)
        """
        try:
            ...
            # v5.25: 거래대금 10억으로 완화 (50억→10억, WAXP 44억/ZETA 14억 포착)
            # 상승률 +2% 이상 (3%→2%, 초기 급등 포착)
            pre_candidates = []
            for t in _tickers:
            ...
            for coin, ticker_chg, ticker_trade_price in scan_list:
                ...
                try:
                    ...
                    closes = list(ohlcv_5m['close'])
                    opens = list(ohlcv_5m['open'])
                    highs = list(ohlcv_5m['high'])
                    volumes = list(ohlcv_5m['volume'])
                    ...
                    # 1분봉 3개 중 2개 이상 음봉이면 이미 꺾임
                    try:
                        ohlcv_1m = pyupbit.get_ohlcv(ticker, interval="minute1", count=3)
                        if ohlcv_1m is not None and len(ohlcv_1m) >= 3:
                            bearish = sum(1 for _, r in ohlcv_1m.iterrows() if r['close'] < r['open'])
                            if bearish >= 2:
                                print(f"  🚫 {coin} 1분봉 {bearish}/3 음봉 → 꺾임, 패스")
                                continue
                    except:
                        pass

                    # 윗꼬리 60%+ → 매도압력 강함
                    _upper = highs[-1] - max(closes[-1], opens[-1])
                    _range = highs[-1] - min(closes[-1], opens[-1])
                    if _range > 0 and _upper / _range >= 0.6:
                        print(f"  🚫 {coin} 윗꼬리 {_upper/_range*100:.0f}% → 매도압력, 패스")
                        continue
        ...
        try:
            ...
            for coin in list(self._surge_watchlist.keys()):
                ...
                if watch.get('entry_ready'):
                    ...
                    # v5.25: surge는 모멘텀 점수 미사용 — 감지 단계에서 이미 3조건 통과
                    _m_score = 0  # surge는 모멘텀 불필요

                    # v5.25: 1분봉 양봉 확인만 (모멘텀/RSI 체크 제거)
                    try:
                        _entry_1m = pyupbit.get_ohlcv(f"KRW-{coin}", interval="minute1", count=2)
                        if _entry_1m is not None and len(_entry_1m) >= 2:
                            if _entry_1m['close'].iloc[-1] < _entry_1m['open'].iloc[-1]:
                                print(f"🚀 [SURGE 대기] {coin} 1분봉 음봉 → 반등 대기")
            ...
            for coin in list(self._surge_positions.keys()):
                ...
                elif elapsed_min < 5 and profit_rate > -0.007:
                    # 수익이어도 5분까지 대기 (5-8분 스위트스팟)
                    # 단, +1% 넘으면 수익 보호로 전환
                    if profit_rate >= 0.01 and peak_rate >= 0.01:
                        _trail = peak_rate - profit_rate
                        if _trail >= 0.003:
                            sell_reason = f"초기 퀵엑싯 (고점{peak_rate*100:+.1f}%→{profit_rate*100:+.1f}%)"
                    if sell_reason is None:
                        print(f"  🚀 [SURGE] {coin}: {profit_rate*100:+.2f}% | {elapsed_min:.1f}분 | 5분 대기")
                        continue
    ...
    def monitor_positions(self):
        ...
        for coin in list(self.positions.keys()):
            ...
            for batch_id in list(self.positions[coin].keys()):
                ...
                # === v6.4: 거래량 추세 + 최대 보유 시간 캡 ===
                # 데이터: 30~60분 승률 54.5% 최고, 60분+ 191건 -27.6만 최악
                # 5분봉 거래량이 항상 30+ → 거래량만으로는 탈출 안 됨
                # → 매수가 대비 방향 + 모멘텀 + 최대 60분 캡
                _vol_trend = "unknown"
                _vol_ratio = 0
                try:
                ...
                # 2. 소액 손실(-0.3%) + 30분 이내 → 회복 대기 (데이터: 22:1 회복)
                if -0.003 <= profit_rate < 0 and minutes_held < 30 and _vol_trend not in ("dead", "selling"):
                    continue

                # 3. 매도세(selling) or 소멸(dead) → 즉시 탈출
                if _vol_trend in ("dead", "selling") and minutes_held >= 10:
                    print(f"📉 {coin} {batch_id} {_vol_trend} → 탈출 ({profit_rate*100:+.2f}%, {minutes_held:.0f}분)")
                    self.place_sell_order(coin, batch_id)
                    self._sell_cooldown[coin] = {'time': time.time(), 'stoploss': profit_rate < 0, 'exit_price': price}
                    continue

                # 4. 모멘텀 죽음 + 손실 → 탈출 (15분 이상)
                if not _mom_alive and profit_rate < -0.003 and minutes_held >= 15:
                    print(f"📉 {coin} {batch_id} 모멘텀↓({momentum:.0f}) + 손실 → 탈출 ({profit_rate*100:+.2f}%, {minutes_held:.0f}분)")
                    self.place_sell_order(coin, batch_id)
                    self._sell_cooldown[coin] = {'time': time.time(), 'stoploss': True, 'exit_price': price}
                    continue

                # 5. 30분+ 횡보(±0.5%) + 모멘텀 약 → 스왑 or 정리
                if abs(profit_rate) < 0.005 and minutes_held >= 30 and not _mom_alive:
                    ...
                    if better and better[0] not in self.positions:
                        print(f"🔄 {coin} {batch_id} 횡보 {minutes_held:.0f}분 → {better[0]}({better[1]}점)으로 교체")
                        ...
                        print(f"🔚 {coin} {batch_id} 횡보 {minutes_held:.0f}분 + 모멘텀{momentum:.0f}↓ → 정리")
                        self.place_sell_order(coin, batch_id)
                        self._sell_cooldown[coin] = {'time': time.time(), 'stoploss': False}
                        continue
