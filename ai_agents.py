...
{{"verdict": "sell|hold", "confidence": 0.0~1.0, "reason": "한줄 근거", "hold_target": 추가 홀딩 시 예상 수익률(%)}}"""
    ...
    def _build_prompt(self, coin, indicators, lessons):
        ...
        return f"""당신은 같은 회사 트레이딩팀 5인입니다. **같은 팀, 같이 살아남기 위해** 프로답게 논의합니다.

규칙:
1. 제공된 숫자 외엔 어떤 수치도 만들지 마시오
2. RSI=0/거래량=0은 "지표 미수신"이지 "데이터 없음"이 아님 → 차단 사유 X
3. 모멘텀 통과 시 1차 검증 완료. 이 토론은 "치명적 리스크 유무"만 판정
4. 감정/자존심 X — 각자 전문 관점에서 객관적 근거만
5. 3:2 이상 찬성 → 매수. 소수의견도 기록 (학습용)
...
팀원 5명 각자 전문 관점에서 (찬/반 + 객관 근거):
1. 🎯 김트레이드 (모멘텀 추세) — 거래량+RSI 기반
2. 🧮 이퀀트 (통계 알고) — 백테스트 확률, 숫자만
3. 🐢 박터틀 (보수 대형추세) — 추세 유효성
4. ⚡ 최스캘퍼 (단타) — 5분 내 반등 가능성
5. 🛡 정디펜더 (리스크) — 최악 시나리오 점검

각자 전문성 발휘, 투표 집계, 팀 결론 도출.
            ...
            if match:
                ...
                if votes and isinstance(votes, dict):
                    ...
                    for trader, vote in votes.items():
                        ...
                        w = max(0.25, w)  # v8.0.1: 최소 0.25 유지 (팀원 존중, 완전 배제 X)
                        if vote == 'buy':
                            buy_weight += w
                        else:
                            skip_weight += w
...
class AIAgents:
    ...
    def get_trader_weights(self):
        """v8.0.1: 각 트레이더 예측 정확도 → 가중치 (학습용, 해고 아님)
        - 맞혀온 트레이더는 발언력 ↑
        - 틀려온 트레이더는 발언력 ↓ (복귀 가능)
        - 팀 전체 집단지성 최적화 목적"""
        try:
